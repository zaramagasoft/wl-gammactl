	...
	while (1) {
		...
		if (fds[1].revents & POLLIN) {
			...
			if (n > 0) {
				...
				// 1. ¿Es una consulta de estado?
				if (buf[0] == 'q') {
					char reply[64];
					// Preparamos la respuesta con los valores actuales
					snprintf(reply,
							sizeof(reply),
							"v %f %f %f",
							brightness,
					...
					printf("DEBUG: Recibi 'q', respondiendo con: %s\n", reply);
					fflush(stdout); // Asegura que se imprima ya mismo
					write(fifo_fd, reply, strlen(reply));
				}
				// 2. ¿Es un comando de ajuste?
				else {
					char cmd;
					double val;
